...
export default function KitchenManagerDashboard() {
  ...
  // Filter reports relevant to kitchen management
  const kitchenReports = reports.filter(report => 
    report.form?.title?.includes('Kitchen') || 
    report.form?.title?.includes('Daily') ||
    report.form?.title?.includes('Equipment')
  );
  ...
  return (
    <div className="p-8">
      <h1 className="text-3xl font-bold mb-6">Kitchen Manager Dashboard</h1>
      <p className="text-gray-600 mb-6">View and manage kitchen inspection reports</p>
      <div className="mb-6">
      ...
      <div className="mb-6 p-4 bg-blue-50 rounded-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white p-4 rounded shadow">
            <p className="text-2xl font-bold">{kitchenReports.length}</p>
            <p className="text-gray-600">Total Kitchen Reports</p>
          </div>
          <div className="bg-white p-4 rounded shadow">
            <p className="text-2xl font-bold">
              {kitchenReports.filter(r => r.status === 'APPROVED').length}
            </p>
            <p className="text-gray-600">Approved Reports</p>
          </div>
          <div className="bg-white p-4 rounded shadow">
            <p className="text-2xl font-bold">
              {kitchenReports.filter(r => r.aiEvaluation === 'POOR').length}
            </p>
            <p className="text-gray-600">Reports Needing Attention</p>
          </div>
        </div>
      </div>
